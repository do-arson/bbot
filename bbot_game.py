...
class Game:
	...
	def updateAllSlices(self, p, add):
		self.updateSlice(p, 0, add)
		self.updateSlice(p, 1, add)
		if not (p.x == 0 and p.y == Game.height - 1) and not (p.x == Game.width - 1 and p.y == 0):
			self.updateSlice(p, 2, add)
		if not (p.x == 0 and p.y == 0) and not (p.x == Game.width - 1 and p.y == Game.height - 1):
			self.updateSlice(p, 3, add)
